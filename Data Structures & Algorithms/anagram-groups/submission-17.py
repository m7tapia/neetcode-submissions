class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for i, string in enumerate(strs):
            ...
            #use # delimiter to avoid some different strs appearing the same (when            a character appears up to double digits, we need some boundary to know)
...
#note: I could also sort it instead of doing the frequency array but sorting would be less efficient.
#The frequency array is O(n) time while sorting is O(nlogn)